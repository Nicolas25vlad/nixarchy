...
let
  ...
  jsonStr = builtins.toJSON;

  menuRow =
    name: app:
    let
      enabledCheck = "grep -qE '^[[:space:]]*${name}\\.enable' $HOME/.config/nixarchy/apps.nix";
    in
    if app ? unavailable then
      # Left visible but permanently dim, with the reason as its subtitle:
      # silently dropping the row would leave a user wondering where an app
      # they know Omarchy offers has gone.
      ''${jsonStr app.menuId}: {"disabled":"true","description":${jsonStr "Not available on NixOS — ${app.unavailable}"}},''
    else
      ''${jsonStr app.menuId}: {"action":${jsonStr "nixarchy-app-enable ${name}"},"disabled":${jsonStr enabledCheck},"description":"Enable in ~/.config/nixarchy/apps.nix, then Apply changes"},'';

  menuRows = lib.concatStringsSep "\n" (
    lib.mapAttrsToList menuRow (lib.filterAttrs (_: a: a ? menuId) apps)
  );

  menuExtension = pkgs.writeText "nixarchy-omarchy-menu.jsonc" ''
    // Generated by nixarchy. Managed declaratively -- edit
    // programs.nixarchy.menu.extraEntries in your NixOS configuration rather
    // than this file, which is a read-only symlink into the Nix store.
    //
    // Every row below overrides one of upstream's by id.
    {
      // Package management is Nix's job. `omarchy-pkg-install` runs pacman and
      // `omarchy-pkg-aur-install` runs yay; neither exists here, and neither
      // has a declarative equivalent worth pretending to.
      "install.package": {"icon":"","label":"Edit app selection","action":"omarchy-launch-editor ''${XDG_CONFIG_HOME:-$HOME/.config}/nixarchy/apps.nix","description":"Every Omarchy app, as NixOS options"},
      "install.aur": {"when":"false"},

      // Nothing is built until this runs, so several apps can be picked first.
      "install.apply": {"icon":"","label":"Apply changes","action":"omarchy-launch-floating-terminal-with-presentation nixarchy-apply","description":"Copy the selection into your flake and nixos-rebuild switch"},

    ${menuRows}
    ${lib.concatStringsSep "\n" (
      lib.mapAttrsToList (id: v: "  ${jsonStr id}: ${jsonStr v},") cfg.menu.extraEntries
    )}
    }
  '';
